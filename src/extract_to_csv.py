# extract_to_csv placeholder
# -*- coding: utf-8 -*-
import csv
from pathlib import Path
from config import SKIP_BASENAMES
from common import (discover_lang_files, rel_from_root, component_from_path,
...
def main():
    ...
    for php in files:
        ...
        for m in RE_SQ.finditer(data):
            key = m.group("k"); text = unescape_php(m.group("t"), "'")
            rows.append([component, relfile, php.name, key, text, "", "pending", sha_row(component, relfile, key, text)])
        # double quoted
